...
def get_normalized_input(weightType: str, min_val: float, max_val: float) -> float:
    ...
    while True:
        try:
            value = float(input(f"Enter your desired {weightType} value: "))
...
def filter_phones_by_price(phone_specs, max_price):
    """Filter phones that exceed the price threshold"""
    filtered_phones = [phone for phone in phone_specs if phone['Price'] <= max_price]
    print(f"Filtered from {len(phone_specs)} to {len(filtered_phones)} phones under ${max_price}")
    if len(phone_specs) < 2:
        
        print("Warning: Not enough phones within budget to perform analysis.")
        print("minimum accepted value is")
...
def run_mcdm_analysis_mobile(phone_specs, criteria_for_decision_phones): #NEW MOBILE
    ...
    #phone_specs = filter_phones_by_price(phone_specs, max_budget)
...
def get_phone_specs():
    ...
    print("Phone specs = " , phone_specs_dict[0:2])
...
if __name__ == "__main__":
    ...
    #del phone_specs, phone_specs_raw
    criteria_for_decision_phones = ["Weight", "RAM", "Battery mAh", "Front Camera", "Back Camera", "Screen Size"] #removed Price
    
    print("Running the MCDM Analysis:")
    ranked_results_phones = run_mcdm_analysis_mobile(phone_specs, criteria_for_decision_phones)
    del phone_specs, phone_specs_raw
    # ✅ FIX: Move this inside the main block with proper indentation
